...
- Special purpose concepts: The concept-vector idea may be extendable to special purposes. For instance, programming concerns could potentially be embedded directly into the core vectors.
  ...
  - Smaller spaces like this don't actually need model distillation. They can be specified manually and injected into the vector-database or even joined directly with pre-existing distilled vectors where relevant via a vector component sum